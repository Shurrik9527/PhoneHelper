...
import com.sharkwang8.phoneassistant.App;
...
import com.sharkwang8.phoneassistant.util.BitmapUtil;
import com.sharkwang8.phoneassistant.util.ScreenUtils;
 ...
 */
public class JunkCleanerView extends View{
    ...
    private int[] mOtherDrawable = {
            R.mipmap.type_unknown, R.mipmap.type_note,
            R.mipmap.type_config, R.mipmap.type_html,
            R.mipmap.type_xml, R.mipmap.type_note,
            R.mipmap.type_pdf, R.mipmap.type_package,
            R.mipmap.type_pic, R.mipmap.type_music,
            R.mipmap.type_video, R.mipmap.type_apk
    };

    public JunkCleanerView(Context context) {
        this(context, null);
    }

    public JunkCleanerView(Context context, @Nullable AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public JunkCleanerView(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
    ...
    private void init(Context context) {
        mContext = context;
        //获取垃圾桶参数
        mDustbinBitmap = BitmapUtil.getBitmapFromRes(context, R.mipmap.junkcheaner_recycle_icon);
    ...
    public void startAnimation() {
        ...
        final ValueAnimator recyclerAnimator = ObjectAnimator.ofFloat(0, 1f);
        final ValueAnimator shakeAnimator = ObjectAnimator.ofFloat(0, -1f, 0, 1f, 0);
    ...
    private void upDateProcessInfo() {
        List<AppProcessInfornBean> mProcessList = ProcessManager.getInstance().getRunningAppList();
    ...
    public JunkCleanerView setFadeTime(int fadeTime) {
        this.mFadeTime = fadeTime;
        return this;
    }

    public JunkCleanerView setRecyclerTime(int recyclerTime) {
        this.mRecyclerTime = recyclerTime;
        return this;
    }

    public JunkCleanerView setShakeTime(int mShakeTime) {
        this.mShakeTime = mShakeTime;
        return this;
    }

    public JunkCleanerView setShakeCount(int mShakeCount) {
        this.mShakeCount = mShakeCount;
        return this;
    }

    public JunkCleanerView setShakeDegree(int shakeDegree) {
    ...
    private class IconInfo {
        ...
        public IconInfo(Bitmap bitmap) {
            ...
            if (nextRandom == 0) {
                ...
                direction = 1;
                startX = ScreenUtils.getScreenWidth();
            }
            controlPoint = new Point();
            controlPoint.x = ScreenUtils.getScreenWidth()/ 2;
            controlPoint.y = (int) (random.nextFloat() * 0.5 * ScreenUtils.getScreenHeight());
        }
